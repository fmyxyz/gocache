...
func TestNewFreecache(t *testing.T) {
	...
	// When
	store := NewFreecache(client, Expiration(6*time.Second))
...
func TestFreecacheGetWithTTL(t *testing.T) {
	...
	store := NewFreecache(client, Expiration(3*time.Second))
...
func TestFreecacheSet(t *testing.T) {
	...
	s := NewFreecache(client, Expiration(6*time.Second))
	err := s.Set(cacheKey, cacheValue, Expiration(6*time.Second))
	assert.Nil(t, err)
}
